---
title: "SQLiteのCRUD処理を sqlite3 と SQLAlchemy で簡単にまとめてみた"
emoji: "🔰"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["python", "sqlite3", "sqlalchemy", "database", "初心者向け"]
published: true
---

# SQLiteのCRUD処理を sqlite3 と SQLAlchemy で簡単にまとめてみた

"sqlite3"と、"sqlalchemy"を使って、SQLiteのCRUD処理を簡単にまとめてみます。
CRUDは、Create・Read・Update・Delete の4つの基本操作の頭文字です。
両者を比較して、使いやすい方を選ぶと良いと思います。

SQLiteは、1つのファイルとしてデータベースを管理します。
出力されたファイルは、次のような専用のソフトウェアで中身を確認することができます。
[DB Browser for SQLite](https://sqlitebrowser.org/)

## 1, SQLite3を使ったCRUDパターン

先ずは、"sqlite3"を使ったCRUDパターンです。
"sqlite3"は、Python標準ライブラリのため、通常はインストール不要です。

### 1-1, モジュールを作る

モジュールにして別ファイルにしておきます。
...
# 2, sqlalchemyを使ったCRUDパターン

次は、"sqlalchemy"を使ったCRUDパターンです。
"sqlalchemy"は、あらかじめインストールが必要です。
