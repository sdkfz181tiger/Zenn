...
![](/images/articles/815885c2c48d12/02.gif)
*物理エンジン*
...
![](/images/articles/815885c2c48d12/03.png)
*Pyxel*
...
Shapeオブジェクトに対し、```shape.mass```で重さを、
```shape.friction```で摩擦力を指定しています。
...
![](/images/articles/815885c2c48d12/05.gif)
*Ragdoll*

# 08: モーターを使って自動車をつくる

物体に回転速度を与える事で、自動車をつくることができます。
次の例では、```pymunk.SimpleMotor()```を使ってモーターを作っています。
(キーボードのAキーで左へ、Dキーで右へ進みます)
...
![](/images/articles/815885c2c48d12/06.gif)
*Car*

# 09: さいごに

今回はPymunkとPyxelを組み合わせた2D物理エンジンを簡単にご紹介しました。
```pymunk.SpaceDebugDrawOptions```を継承したクラスを用意することで、
それぞれの描画ライブラリに合わせたデバッグ描画を簡単に実装できます。

描画処理で使った"Pyxel"は、とても扱いやすく(しかも可愛い)おすすめフレームワークです。
よろしければ[PythonでPyxelをかじる本](https://zenn.dev/sdkfz181tiger/books/675e49a3e4d34b)もぜひぜひご参照くださいませ。
