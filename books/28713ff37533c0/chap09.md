...
今回は、取得した小判の数をスコアとして画面に表示します。

## 1, テキストオブジェクトを作る

"main.py"にある、"GameView"クラスのコンストラクタで、数値としてのスコアと、"arcade.Text()"メソッドで、テキストオブジェクトを用意します。
引数にはそれぞれ、以下の情報を指定します。
- 表示するテキスト
- x座標 / y座標
- 文字色
- 文字サイズ
- x方向の基準点(テキストの水平中央を基準点)
- y方向の基準点(テキストの垂直上部を基準点)
...
複数の小判に同時に触れた場合は、その分まとめてスコアが加算されます。
...
![](/images/books/28713ff37533c0/09_01.gif)
