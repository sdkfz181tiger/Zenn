...
"main.py"では、Pyxelエンジン等、必要なモジュールをインポートします。
以降、"Game"クラスに具体的な実装を進めます。
...
"Game"クラスのコンストラクタで、ゲームオブジェクトを作ります。
...
次に続く、"pyxel.run()"の引数で、次の2つのメソッドを指定します。
- 更新処理のメソッド
- 描画処理のメソッド
...
キャラクター等の描画処理を"self.draw()"メソッドに記述します。
