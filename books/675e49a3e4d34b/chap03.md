...
"Pyxel Editor"は、画像やサウンド等のデータをまとめて管理するツールです。
これらのデータは、"pyxres"形式のファイルにまとめて管理します。

エディタの起動は次のコマンドから立ち上げる事ができます。
("$"マークはコマンドに含めません)
...
"Pyxel Editor"の使い方に関しては、以下のサイトの解説がとても丁寧です。
...
今回は、次の様に宇宙船x2、隕石x6、ブースターx2を描きました。

![](/images/675e49a3e4d34b/03_02.png)

## 3, pyxresファイルを配置する

生成された"pyxres"ファイルを、作業用フォルダに配置します。
フォルダ構成は次の様になります。
...
配置した"pyxres"ファイルを読み込む処理を実装します。
"Game"クラスのコンストラクタで、次の様にしてファイルを読み込みます。
