...
"MyDB"クラスに、CRUD処理を実装していきます。

## 5-1, データの追加(Create)

"insert_record()"メソッドでは、テーブルにデータを追加します。

``` with self.db_session() as session: ```に注目してください。
この部分で一時的にセッションを作り、テーブルにデータを追加、コミットを行います。
これらの処理の実行後、自動的にセッションがクローズされます。
...
"update_record()"メソッドでは、テーブルのデータを更新します。
...
"delete_record()"メソッドでは、テーブルのデータを削除します。

"session.delete(record)"で削除します。
